    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
        <div className="flex justify-between items-center mb-6">
          <h3 className="font-semibold text-slate-800 text-lg">Active Crops</h3>
          <div className="flex gap-4 items-center">
            <button 
              onClick={() => setIsAddingCrop(true)} 
              className="text-green-600 hover:text-green-700 text-sm font-medium flex items-center gap-1 focus:outline-none focus:ring-2 focus:ring-green-500 rounded-md p-1 transition-colors"
            >
              <Plus className="w-4 h-4" /> Add Crop
            </button>
            <button onClick={() => onNavigate?.('erp')} aria-label="Manage all crops" className="text-green-600 text-sm font-medium hover:underline focus:outline-none focus:ring-2 focus:ring-green-500 rounded-md p-1">Manage All</button>
          </div>
        </div>

        {isAddingCrop && (
          <form onSubmit={handleAddCrop} className="mb-6 p-5 rounded-2xl bg-green-50/40 border border-green-100 flex flex-col md:flex-row gap-4 items-end animate-fadeIn">
            <div className="flex-1 space-y-1 w-full">
              <label className="text-xs font-semibold text-slate-500">Crop Name</label>
              <input 
                type="text" 
                required
                value={newCropName}
                ...
                className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-green-500 focus:outline-none text-slate-700"
              />
            </div>
            <div className="w-full md:w-36 space-y-1">
              ...
              <select 
                value={newCropStatus}
                onChange={(e) => setNewCropStatus(e.target.value as 'Healthy' | 'Attention')}
                className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-green-500 focus:outline-none text-slate-700"
              >
            ...
            <div className="w-full md:w-40 space-y-1">
              <label className="text-xs font-semibold text-slate-500 flex justify-between">
                <span>Progress</span>
                <span className="font-bold text-green-700">{newCropProgress}%</span>
              </label>
              <input 
                type="range" 
                min="0" 
                max="100"
                value={newCropProgress}
                onChange={(e) => setNewCropProgress(Number(e.target.value))}
                className="w-full accent-green-600 cursor-pointer h-2 bg-slate-200 rounded-lg appearance-none py-2"
              />
            </div>
            <div className="flex-1 space-y-1 w-full">
              <label className="text-xs font-semibold text-slate-500">Next Action</label>
              <input 
                type="text" 
                required
                value={newCropAction}
                onChange={(e) => setNewCropAction(e.target.value)}
                placeholder="e.g. Irrigation in 3 days" 
                className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-green-500 focus:outline-none text-slate-700"
              />
            </div>
            <div className="flex gap-2 shrink-0 w-full md:w-auto">
              <button 
                type="button" 
                onClick={resetAddCropForm}
                className="flex-1 md:flex-none px-4 py-2 border border-slate-200 text-slate-600 rounded-xl text-sm hover:bg-slate-50 transition-colors"
              >
                Cancel
              </button>
              <button 
                type="submit" 
                className="flex-1 md:flex-none px-4 py-2 bg-green-600 text-white rounded-xl text-sm hover:bg-green-700 transition-colors"
              >
                Save
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {crops.length === 0 ? (
            <div className="col-span-1 md:col-span-3 py-10 text-center text-slate-400">
              <p className="text-sm">No active crops registered.</p>
              <p className="text-xs mt-1">Click "Add Crop" above to register your crops.</p>
            </div>
          ) : (
            crops.map((crop, i) => (
              <div key={i} className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-center gap-4 relative group hover:bg-slate-100/60 transition-colors">
                <div className="w-12 h-12 rounded-xl bg-green-100 flex items-center justify-center shrink-0">
                  <Sprout className="w-6 h-6 text-green-600" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-1 gap-2">
                    <h4 className="font-semibold text-slate-800 truncate">{crop.name}</h4>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${
                        crop.status === 'Healthy' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'
                      }`}>
                        {crop.status}
                      </span>
                      <button 
                        onClick={() => handleRemoveCrop(i)}
                        className="p-1 hover:bg-red-50 text-slate-400 hover:text-red-600 rounded-lg transition-colors md:opacity-0 group-hover:opacity-100 focus:opacity-100"
                        title="Remove crop"
                      >
                  ...
                  <div className="w-full bg-slate-200 rounded-full h-1.5 mb-2">
                    <div className="bg-green-500 h-1.5 rounded-full" style={{ width: `${crop.progress}%` }}></div>
                  </div>
                  <p className="text-xs text-slate-500 truncate">{crop.nextAction}</p>
                </div>
              </div>
            ))
          )}
